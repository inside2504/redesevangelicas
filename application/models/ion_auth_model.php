...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db->select('ContraUsuario, Salt')
		                  ->where('AidiUsuario', $id)
		                  ->limit(1)
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			if ($this->bcrypt->verify($password,$hash_password_db->ContraUsuario))
			{
				return TRUE;
			}
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['Usuario'], $data, array('AidiUsuario' => $id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db->where('CodeContraOlvidada', $code);

		if ($this->db->count_all_results($this->tables['Usuario']) > 0)
		{
			$data = array(
			    'CodeContraOlvidada' => NULL,
			    'TiempoContraOlvidada' => NULL
			);

			$this->db->update($this->tables['Usuario'], $data, array('CodeContraOlvidada' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db->select('AidiUsuario, ContraUsuario, Salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$data = array(
			    'ContraUsuario' => $hashed_new_password,
			    'Code_Recordatorio' => NULL,
			);

			$this->trigger_events('extra_where');

			$successfully_changed_password_in_db = $this->db->update($this->tables['Usuario'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
										->group_by("AidiUsuario")
										->order_by("AidiUsuario", "ASC")
										->limit(1)
	...
	public function email_check($email = '')
	{
		...
		return $this->db->where('CorreUser', $email)
										->group_by("AidiUsuario")
										->order_by("AidiUsuario", "ASC")
										->limit(1)
		                ->count_all_results($this->tables['Usuario']) > 0;
	...
	public function forgotten_password_complete($code, $salt=FALSE)
	{
		...
		$profile = $this->where('CodeContraOlvidada', $code)->users()->row(); //pass the code to profile
		...
		if ($profile) {
			...
			$data = array(
			    'ContraUsuario'                => $this->hash_password($password, $salt),
			    'CodeContraOlvidada' => NULL,
			    'Activo'                  => 1,
			 );
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['Grupos'],array('NeimGrupo' => $this->config->item('default_group', 'ion_auth')),1)->row();
		...
		$user_data = array_merge($this->_filter_data($this->tables['Usuario'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['Usuario'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			$password = $this->hash_password_db($user->AidiUsuario, $password);
			...
			if ($password === TRUE)
			{
				...
				$this->update_last_login($user->AidiUsuario);

				$this->clear_login_attempts($identity);

				if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{
					$this->remember_user($user->AidiUsuario);
				}
	...
	function get_attempts_num($identity)
	{
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            if ($this->config->item('track_login_ip_address', 'ion_auth')) {
            	$this->db->where('DirIp', $ip_address);
            	$this->db->where('LoginInt', $identity);
            } else if (strlen($identity) > 0) $this->db->or_where('LoginInt', $identity);
            $qres = $this->db->get($this->tables['Intentos_Login']);
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            if ($this->config->item('track_login_ip_address', 'ion_auth')) $this->db->where('DirIp', $ip_address);
			else if (strlen($identity) > 0) $this->db->or_where('LoginInt', $identity);
			$qres = $this->db->get($this->tables['Intentos_Login'], 1);
	...
	public function increase_login_attempts($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->db->insert($this->tables['Intentos_Login'], array('DirIp' => $ip_address, 'LoginInt' => $identity, 'HoraInt' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db->where(array('DirIp' => $ip_address, 'LoginInt' => $identity));
			// Purge obsolete login attempts
			$this->db->or_where('HoraInt <', time() - $expire_period, FALSE);

			return $this->db->delete($this->tables['Intentos_Login']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['Usuario'].'.*',
			    $this->tables['Usuario'].'.AidiUsuario as id',
			    $this->tables['Usuario'].'.AidiUsuario as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['Grupos_Usuarios'],
				    $this->tables['Grupos_Usuarios'].'.'.$this->join['Usuario'].'='.$this->tables['Usuario'].'.AidiUsuario',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['Grupos'], $this->tables['Grupos_Usuarios'] . '.' . $this->join['Grupos'] . ' = ' . $this->tables['Grupos'] . '.AidiGrupo', 'inner');
				$this->db->where_in($this->tables['Grupos'] . '.NeimGrupo', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['Grupos_Usuarios'].'.'.$this->join['Grupos'], $group_ids);
			}
		}
	...
	public function user($id = NULL)
	{
		...
		$id || $id = $this->session->userdata('AidiUsuario');

		$this->limit(1);
		$this->order_by($this->tables['Usuario'].'.AidiUsuario', 'desc');
		$this->where($this->tables['Usuario'].'.AidiUsuario', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['Grupos_Usuarios'].'.'.$this->join['Grupos'].' as AidiGrupo, '.$this->tables['Grupos'].'.NeimGrupo, '.$this->tables['Grupos'].'.DescripcioN')
		                ->where($this->tables['Grupos_Usuarios'].'.'.$this->join['Usuario'], $id)
		                ->join($this->tables['Grupos'], $this->tables['Grupos_Usuarios'].'.'.$this->join['Grupos'].'='.$this->tables['Grupos'].'.AidiGrupo')
		                ->get($this->tables['Grupos_Usuarios']);
	}
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		// if no id was passed use the current users id
		$user_id || $user_id = $this->session->userdata('AidiGrupo');
		...
		{
			if ($this->db->insert($this->tables['Grupos_Usuarios'], array( $this->join['Grupos'] => (float)$group_id, $this->join['Usuario'] => (float)$user_id)))
			{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			foreach($group_ids as $group_id)
			{
				$this->db->delete($this->tables['Grupos_Usuarios'], array($this->join['Grupos'] => (float)$group_id, $this->join['Usuario'] => (float)$user_id));
		...
		{
			if ($return = $this->db->delete($this->tables['Grupos_Usuarios'], array($this->join['Usuario'] => (float)$user_id))) {
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['Grupos'].'.AidiGrupo', $id);
		}

		$this->limit(1);
		$this->order_by('AidiGrupo', 'desc');
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['Usuario'], $data);

		if (array_key_exists($this->identity_column, $data) || array_key_exists('ContraUsuario', $data) || array_key_exists('ContraUsuario', $data))
		{
			if (array_key_exists('ContraUsuario', $data))
			{
				if( ! empty($data['ContraUsuario']))
				{
					$data['ContraUsuario'] = $this->hash_password($data['ContraUsuario'], $user->salt);
				}
				else
				{
					// unset password so it doesn't effect database entry if no password passed
					unset($data['ContraUsuario']);
				}
			}
		}

		$this->trigger_events('extra_where');
		$this->db->update($this->tables['Usuario'], $data, array('AidiUsuario' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['Usuario'], array('AidiUsuario' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['Usuario'], array('UltimaVez' => time()), array('AidiUsuario' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['Usuario'], array('Code_Recordatorio' => $salt), array('AidiUsuario' => $id));
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['Grupos'], array('NeimGrupo' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$data = array('NeimGrupo'=>$group_name,'DescripcioN'=>$group_description);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['Grupos'], array('NeimGrupo' => $group_name))->row();
			...
			$data['NeimGrupo'] = $group_name;
		}

		// restrict change of name of the admin group
        $group = $this->db->get_where($this->tables['Grupos'], array('AidiGrupo' => $group_id))->row();
		...
		$this->db->update($this->tables['Grupos'], $data, array('AidiGrupo' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['Grupos_Usuarios'], array($this->join['Grupos'] => $group_id));
		// remove the group itself
		$this->db->delete($this->tables['Grupos'], array('AidiGrupo' => $group_id));
